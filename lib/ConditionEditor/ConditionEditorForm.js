  ...
  function (declare,
    ...
    return declare([], {
      ...
      constructor: function (conf, domNode, filteredStoreDataOption) {
        ...
        this.filter.startup();
        dom.byId(domNode)
          .appendChild(this.filter.domNode);
            ...
            onClick: function (event) {
              if (this.targetNode) {
                if (this.targetNode.mayHawChild) {
                  ...
                  alert('Condition can`t have child nodes');
                }
              } else {
      ...
      _conditionEditDialogInit: function () {
        ...
        if (!self.conditionEditorDialog) {
          ...
          self.conditionEditorDialog = new ConfirmDialog({
            ...
            title: 'Filter',
            style: 'width:600px; z-index: 9999',
            content: self.conditionEditorDialogForm,
            execute: function () {
              var f = self.conditionEditorDialogForm.value,
                filter = f['filter'],
                field = f['field'],
                value = f['value'],
                param = f['type'] !== undefined ? f['type'] : null;
              ...
              var currentFilter = new SmartFilterNode(filter);
              currentFilter.mayHawChild = false;
              currentFilter.name = field;
              currentFilter.value = parser(value, param);
