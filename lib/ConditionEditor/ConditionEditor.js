  ...
  function (declare,
    ...
    return declare([_WidgetBase, _TemplatedMixin], {
        ...
        this.own(
          ...
          aspect.around(self._store, 'itemCreator', lang.hitch(this, function (originalItemCreator) {
            return function (nodes, target, source) {
              const sourceItem = source.getItem(nodes[0].id);
              let filter = null;
              if (sourceItem) {
      ...
      saveFilter: function (event) {
        ...
        this._store.get(0)
          .then((root) => {
            let filter = parser.getAllChild(this._store, root);
            filter = parser.optimiseFilter(filter, filter.type);
            const lastElementIndex = filter.length - 1;
            event.filter = {
