define([
	...
	'dojo/dom-class',
	'dgrid/Grid'
], function (declare, Deferred, lang, when, on, domClass, Grid) {
	...
	return declare(null, {
		...
		_showEditor: function (cmp, column, cellElement, value) {
			...
			let widgetParentNode;
			if (cmp.domNode) {
				widgetParentNode = cmp.domNode.parentNode;
			}
			cellElement.appendChild(cmp.domNode ? cmp.domNode : cmp);
			// My hack which resets grid cell to it`s original stance when the editor is moved
			// If innerHTML is empty, then custom editor was used and dgrid/Edior didn`t work
			if (widgetParentNode && widgetParentNode.innerHTML === ''){
				const options = {alreadyHooked: true},
					parentCell = this.cell(widgetParentNode),
					column = this.column(widgetParentNode);
				column._editorBlurHandle.pause();
				domClass.remove(parentCell.element, 'dgrid-cell-editing');
				const cell = column.renderCell(parentCell.row.data, this._activeValue, widgetParentNode,
					this._activeOptions ? lang.delegate(options, this._activeOptions) : options);
				// If default renderer is used, 'cell' will be undefined.
				Grid.appendIfNode(widgetParentNode, cell);
			}
