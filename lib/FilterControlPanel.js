define(
    [
        ...
        'dgrid/_StoreMixin',
        'Rscript/FilterEditor/FilterEditorForm',
        'rql/parser',
        'dojo/when',
        ...
        return declare([_WidgetBase, _TemplatedMixin], {
            ...
            constructor: function (object) {
                ...
                if (typeof object.grid === 'object') {
                    this._grid = object.grid;
                } else {
                    throw new TypeError('grid passed to FilterControlPanel is not an object')
                }
                if (typeof object.store === 'object') {
                    this.store = object.store;
                }
                if (typeof object._configStore === 'object') {
                    this._configStore = object._configStore
                }
                this._getFilterEditorData();
            },

            _getFilterEditorData: function () {
                var filterManager = this,
                    columns;
                if (this._configStore) {
                    columns = this._configStore.get('gridColumnDefinitions')
                } else {
                    columns = filterManager._grid.get('columns');
                }
            ...
            _applySelectedFilter: function (filterRow) {
                if (filterRow !== null && filterRow !== undefined) {
                    var queryObject = Parser.parse(filterRow.filter);
                    this._grid.setFilter(queryObject, filterRow.name);
                    this._grid.loadContent();
                } else {
