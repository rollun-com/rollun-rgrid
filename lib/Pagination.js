    ...
    function (declare,
        ...
        return declare("Pagination", [_WidgetBase], {
            ...
            _updateRowsPerPageOption: function () {
                var rowsPerPage = this._rowsPerPage,
            ...
            postCreate: function () {
                ...
                on(this.eventBus, 'gridLoadedContent', function (event) {
                    pagination._updatePaginationStatus(event.totalCount);
                    pagination._updateNavigation(event.totalCount);
                });
                this._performFirstStatusUpdate();
            }
            ,

            _performFirstStatusUpdate: function () {//TODO: think of a better solution
                const pagination = this;
                on.once(this.eventBus, 'gridTotalCount', function (event) {
                    pagination._updatePaginationStatus(event.totalCount);
                    pagination._updateNavigation(event.totalCount);
                });
                this.eventBus.emit('getGridTotalCount');
            }
            ,
            ...
            gotoPage: function (pageNumber) {
                ...
                const limit = this._rowsPerPage,
                    offset = parseInt((pageNumber - 1) * limit),
                    limitNode = new Query.Query({name: 'limit', args: [limit, offset]});
                this.eventBus.emit('loadGridContentForQuery', {query: limitNode});
            }
            ,
        })
            ;
