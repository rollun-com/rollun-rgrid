define([
	...
	function (declare,
		...
		return declare("Pagination", [_WidgetBase, _EventDriven], {
			...
			_currentPage: 1,
			rowsPerPage: 20,
			class: 'r-pagination-widget bg-light',
			_isLoading: false,
			...
			buildRendering: function () {
				...
				this.reloadButton.placeAt(this.paginationNode, 'after');
			},
			...
			_updatePaginationStatus: function (total) {
				...
				this.paginationStatusNode.innerHTML =
					`Showing records  ${start}-${Math.min(total, (start + count - 1))} of ${total}`;
			},
			...
			loadGridContent: function () {
				...
				this.handleEventOnce('gridLoadedContent', () => {
