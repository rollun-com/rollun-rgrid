    ...
    function (declare,
        ...
        return declare([_WidgetBase, _TemplatedMixin], {
            ...
            startup: function () {
                this.inherited(arguments);
                this.grid.startup();
                var dgpr = query("div.dgrid-scroller");
                for (var id in dgpr) {
                    if (dgpr.hasOwnProperty(id) && !isNaN(parseInt(id))) {
                        style.set(dgpr[id], {
                            "margin-top": "25px"
                        });
                    }
            ...
            setStore: function (dStore) {
                var self = this;
                self.grid.set('collection', dStore);
            },
            ...
            _setFilterBlock: function (filterName) {
                var self = this;
                var filterNode = domConstructor.create('button', {'class': "btn btn-warning filter_active"});
                on(filterNode, "click", function () {
                        self.clear();
                        self.filterActiveNode.innerHTML = "";
                        on.emit(self.filterActiveNode, "filterRemovedFromGrid", {bubbles: true});
                    }
                );
                domConstructor.place("<span class='glyphicon glyphicon-remove' style='float:right;'></span><p>" + 'Убрать ' + filterName + "</p>", filterNode);
                self.filterActiveNode.innerHTML = "";
