		...
		return declare([_WidgetBase, _TemplatedMixin, _EventDriven], {
			...
			doSearch: function () {
				...
				when(columnNames, function (columns) {
					...
					search.emit('loadGridContentForQuery', {query: searchQuery, label: "Search " + searchValue});
			...
			/**
			 *
			 * @returns {object}- promise that wil resolve with array containing column definition objects
			 * @private
			 */
			...
			/**
			 * @param searchValue {string} - value from user input
			 * @param columns {array} - column definition objects
			 * @return {object} - rollun-rql query object
			 */
