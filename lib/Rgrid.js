define([
	...
	return declare('Rgrid', [
		...
		_getFilterNode: function (label, nodeKey) {
			...
			domConstruct.place(
				"<div style='display:flex; align-content: center'><span> Убрать " + label + "</span><i class='fas fa-times' " +
				...
				for (const fieldName in response[0]) { //no columns were specified
					if (response[0].hasOwnProperty(fieldName)) {
						let column = {
							...
							hidden: false,
							//editor: 'textarea',
							editor: JsonEditorForRgrid,
							editOn: 'dblclick',
							autoSave: true,
							editorArgs: {grid: this},
						};
						if (this._columnsConfig) {
							column = this._addColumnConfig(column);
						}
