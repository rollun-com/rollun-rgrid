	...
	return declare('Rgrid', [
		...
		constructor: function (params) {
			...
			if (typeof params.columnsConfig === 'object') {
				this._columnsConfig = this._processColumnsConfig(params.columnsConfig);
			}
			this._nodeCache = {};
			this._nodeCache.default = this._query;
		},
		...
		_processColumnsConfig: function (columnsConfig) {
			for (const columnName in columnsConfig) {
				if (columnsConfig.hasOwnProperty(columnName)) {
					columnsConfig[columnName].formatter = window[columnsConfig[columnName].formatter];
					columnsConfig[columnName].renderCell = window[columnsConfig[columnName].renderCell];
					columnsConfig[columnName].get = window[columnsConfig[columnName].get];
					columnsConfig[columnName].renderHeaderCell = window[columnsConfig[columnName].renderHeaderCell];
				}
			}
		...
		_handleColumnChanges: function () {
			...
			on(this.domNode, 'dgrid-columnreorder', (event) => {
				...
				this._nodeCache.select = selectNode;
			});
		...
		_startEmittingColumnSelect: function () {
			on(this.contentNode, 'dgrid-select', (event) => {
				this.emit('grid-select', event);
			});
			on(this.contentNode, 'dgrid-deselect', (event) => {
				this.emit('grid-deselect', event);
			});
		},
		...
		setFilterBlock: function (label, nodeKey) {
			...
			on(filterNode, "click", () => {
					delete this._nodeCache[nodeKey];
		...
		_buildQueryFromCache: function () {
			this.setQuery(this._nodeCache.default);
			for (let nodeKey in this._nodeCache) {
				if (nodeKey !== 'default' && this._nodeCache.hasOwnProperty(nodeKey)) {
